...
# * 代表全部
# main,schedule,
@export var show_category: Array[Enums.LogType] = [
#Enums.LogType.all, # 开启此项会全部显示
# 关闭all后,开启哪项显示哪项
#Enums.LogType.main,
#Enums.LogType.schedule,
# Enums.LogType.saveload,
#Enums.LogType.combo,
#Enums.LogType.supplier,
#Enums.LogType.settlement,
#Enums.LogType.input,
# Enums.LogType.BUSINNESS,
# Enums.LogType.BUY,
]
...
func _ready():
	show_color = !TheDebug.is_vscode()  # vscode无法解析带颜色的字符
	if !DirAccess.dir_exists_absolute(log_dir):
		DirAccess.make_dir_absolute(log_dir)
	prints(
		"---logger ready:",
		"show_level:" + loglevel.keys()[show_level],
		"show_category:" + ",".join(show_category.map(func(i): return Enums.LogType.keys()[i])),
		"---"
	)
...
func _is_category_show(category):
	if show_category.has(Enums.LogType.all):
		return true
	if category == Enums.LogType.all:
		# 默认的也显示
		return true
...
## @param category 日志类型
## @param data 日志数据
func debug(name, data = "", category: Enums.LogType = Enums.LogType.all):
...
func info(name, data = "", category: Enums.LogType = Enums.LogType.all):
...
func warn(name, data = "", category: Enums.LogType = Enums.LogType.all):
...
func error(name, data = "", category: Enums.LogType = Enums.LogType.all):
...
func _get_message(
	name,
	data,
	category: Enums.LogType = Enums.LogType.all,
	level = loglevel.info,
	levelcolor = "orange",
	datacolor = "white"
):
		...
		return color_template.format(
			{
				...
				category = Enums.LogType.keys()[category],
				name = str(name),
				data = str(data),
...
func profiler_start(category: Enums.LogType = Enums.LogType.all):
	if show_level > loglevel.debug or !_is_category_show(category):
		return
	profiler_data.clear()
	is_profiling = true
	_add_profiler_message(
		"--------------profilter start-----------------[{category}]".format(
			{category = Enums.LogType.keys()[category]}
		),
		"",
		category
...
func profiler(name, data = "", category: Enums.LogType = Enums.LogType.all):
	if show_level > loglevel.debug or !_is_category_show(category):
		return
	if is_profiling:
...
func profiler_stop(category: Enums.LogType = Enums.LogType.all):
	if show_level > loglevel.debug or !_is_category_show(category):
		return
	is_profiling = false
	_add_profiler_message(
		"--------------profilter stop-----------------[{category}]".format(
			{category = Enums.LogType.keys()[category]}
		),
		"",
		category
...
func mouse_text(msg):
	...
	TheDebug.add_child(mousetext)
	mousetext.global_position = TheDebug.debugnode2d.get_global_mouse_position()
